...
int main(int argc, char const* argv[])
{
    ...
    for (testno = testnolow; testno <= testnohigh; ++testno)
    {
        ...
        for (hashno = hashnolow; hashno <= hashnohigh; ++hashno)
        {
            ...
            dataarray = (const unsigned char*) (hmactests[testno].dataarray[hashno] ? hmactests[testno].dataarray[hashno] : hmactests[testno].dataarray[1]
                                                                                                                            ? hmactests[testno].dataarray[1]
                                                                                                                            : hmactests[testno].dataarray[0]);  // testarray
            datalength = hmactests[testno].datalength[hashno] ? hmactests[testno].datalength[hashno] : hmactests[testno].datalength[1] ? hmactests[testno].datalength[1]
                                                                                                                                       : hmactests[testno].datalength[0];  // length
            keyarray = (const unsigned char*) (hmactests[testno].keyarray[hashno] ? hmactests[testno].keyarray[hashno] : hmactests[testno].keyarray[1]
                                                                                                                         ? hmactests[testno].keyarray[1]
                                                                                                                         : hmactests[testno].keyarray[0]);  // keyarray
            keylength = hmactests[testno].keylength[hashno] ? hmactests[testno].keylength[hashno] : hmactests[testno].keylength[1] ? hmactests[testno].keylength[1]
                                                                                                                                   : hmactests[testno].keylength[0];  // keylen
            resultarray = (const unsigned char*) (hmactests[testno].resultarray[hashno]);  // resultarray
            resultlength = hmactests[testno].resultlength[hashno];  // resultlength
            ...
            err = cudaMemcpy(dev_keyarray, keyarray, keylength, cudaMemcpyHostToDevice);
            if (err != cudaSuccess)
            {
                printf("cudaMemcpy failed: %s\n", cudaGetErrorString(err));
                return 1;
            }
            err = cudaMemcpy(dev_dataarray, dataarray, datalength, cudaMemcpyHostToDevice);
            if (err != cudaSuccess)
            {
                printf("cudaMemcpy failed: %s\n", cudaGetErrorString(err));
                return 1;
            ...
            case 0:
                ...
                global_hmac_sha1<<<1, 1>>>(dev_dataarray, datalength, dev_keyarray, keylength, dev_HMAC_Digest);
                break;

            case 1:
                printf("\tHMAC-SHA224 test: \n");
                global_hmac_sha224<<<1, 1>>>(dev_dataarray, datalength, dev_keyarray, keylength, dev_HMAC_Digest);
                break;

            case 2:
                printf("\tHMAC-SHA256 test: \n");
                global_hmac_sha256<<<1, 1>>>(dev_dataarray, datalength, dev_keyarray, keylength, dev_HMAC_Digest);
                break;

            case 3:
                printf("\tHMAC-SHA384 test: \n");
                global_hmac_sha384<<<1, 1>>>(dev_dataarray, datalength, dev_keyarray, keylength, dev_HMAC_Digest);
                break;

            case 4:
            {
                printf("\tHMAC-SHA512 test: \n");
                global_hmac_sha512<<<16, 16>>>(dev_dataarray, datalength, dev_keyarray, keylength, dev_HMAC_Digest);
                break;
            }
            default:
                break;
            ...
            err = cudaMemcpy(HMAC_Digest, dev_HMAC_Digest, USHAMaxHashSize, cudaMemcpyDeviceToHost);
            if (err != cudaSuccess)
            {
                printf("cudaMemcpy failed: %s\n", cudaGetErrorString(err));
                return 1;
    ...
    cudaFree(dev_keyarray);
    cudaFree(dev_dataarray);
    cudaFree(dev_HMAC_Digest);
